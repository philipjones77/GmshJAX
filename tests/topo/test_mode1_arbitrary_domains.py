...
def test_sphere_volume_tet_mesh_supports_mode1_and_export(tmp_path: Path) -> None:
...
def test_launch_gmsh_viewer_invokes_external_viewer(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    proc = launch_gmsh_viewer(mesh_path, gmsh_executable="gmsh", wait=True, extra_args=["-nopopup"])
