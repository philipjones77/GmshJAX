"""Benchmark Mode 3 soft-connectivity surrogate optimization."""
...
from topojax.ad.surrogate import optimize_soft_connectivity, summarize_mode3_result
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Benchmark TopoJAX Mode 3 surrogate optimization.")
    ...
    t0 = perf_counter()
    result = optimize_soft_connectivity(points, topo.elements, steps=args.steps, step_size=0.1, temperature=0.25)
    elapsed_s = perf_counter() - t0
    payload = {
        "benchmark": "mode3_surrogate",
        "kind": args.kind,
        "runtime_s": elapsed_s,
        **summarize_mode3_result(result),
    }
