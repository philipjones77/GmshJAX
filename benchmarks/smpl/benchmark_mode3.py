...
from smpljax import default_mode1_params, optimize_mode3
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Benchmark SMPLJAX Mode 3 soft-routing optimization.")
    ...
    first = optimize_mode3(model, params, steps=4, step_size=0.05, logit_step_size=0.02, diagnostics_every=2, activation_weight=0.0)
    first_time_s = time.perf_counter() - t0

    t0 = time.perf_counter()
    for _ in range(args.iters):
        result = optimize_mode3(model, params, steps=4, step_size=0.05, logit_step_size=0.02, diagnostics_every=2, activation_weight=0.0)
