...
from smpljax import default_mode1_params, optimize_mode4
from smpljax.body_models import SMPLJAXModel
from smpljax.utils import SMPLModelData


def _build_synthetic_model(*, num_verts: int, num_joints: int, num_betas: int, dtype: jnp.dtype) -> SMPLJAXModel:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Benchmark SMPLJAX Mode 4 straight-through optimization.")
    ...
    first = optimize_mode4(model, params, steps=4, step_size=0.05, logit_step_size=0.02, diagnostics_every=2, activation_weight=0.0)
    first_time_s = time.perf_counter() - t0

    t0 = time.perf_counter()
    for _ in range(args.iters):
        result = optimize_mode4(model, params, steps=4, step_size=0.05, logit_step_size=0.02, diagnostics_every=2, activation_weight=0.0)
        assert result.objective_history is not None
    avg_runtime_s = (time.perf_counter() - t0) / max(args.iters, 1)

    payload = {
        "benchmark": "mode4",
        "runtime": "straight-through-routing",
        "dtype": args.dtype,
        ...
        "n_groups": len(first.group_summaries),
        "n_steps": int(first.objective_history.shape[0]),
